...
version: 0.2.0
license: MIT
description: PII detection and masking filter for Keeper AI Gateway. Task 3 — Presidio engine + 12 custom recognizers + HR NLP. Detection only (masking comes in Task 4).
...
class Pipeline:
    ...
    async def inlet(
        ...
        """Detect PII in the last user message and attach detections to metadata.

        The body itself is not modified beyond `body["metadata"]["pii_detections"]`.
        Real masking happens in Task 4.
        """
        ...
        try:
            ...
            logger.exception("inlet: analyzer.analyze() failed")
        ...
        for r in results:
            standard = self.PRESIDIO_TO_STANDARD.get(r.entity_type)
            if standard is None:
                logger.debug("inlet: dropping non-whitelisted entity type %r", r.entity_type)
                continue
            detections.append(
                {
